...
from keyword_service import KEYWORD_PROVIDER_PRESETS, DEFAULT_MANGOOLS_API_KEY
from llm_config import DEFAULT_DEEPSEEK_API_KEY
from subscription import trial_days_left, has_used_trial

model_config_bp = Blueprint("model_config", __name__, url_prefix="/dashboard/models")

# ==================== کلیدهای دیفالت (خوانده‌شده از فایل .env) ====================
# این کلیدها برای همه‌ی کاربران به‌صورت پیش‌فرض در دسترس هستند و نیازی به ثبت
# دستی توسط کاربر ندارند؛ کافیست در فایل .env مقدار زیر تنظیم شود:
#   DEEPSEEK_API_KEY=sk-xxxxxxxx
#   MANGOOLS_API_KEY=xxxxxxxx
# اگر کاربر خودش ارائه‌دهنده/ابزاری اضافه و فعال کند، همان بر کلید دیفالت
# اولویت دارد.


def _mask_key(key: str) -> str:
    """نمایش امن بخشی از کلید API (بدون افشای کامل آن)."""
    if not key:
        return ""
    if len(key) <= 8:
        return "•" * len(key)
...
DEFAULT_KEYS_INFO = {
    "deepseek": {
        "display_name": "کلید دیفالت دیپ‌سیک",
        "description": "کلید پیش‌فرض DeepSeek که برای «تحلیل محتوا»، «خلاصه‌ساز» و «جستجوی کلمات کلیدی با هوش مصنوعی» استفاده می‌شود، مگر اینکه ارائه‌دهنده‌ی دیگری فعال کرده باشید.",
        "env_var": "DEEPSEEK_API_KEY",
    },
    "kwfinder": {
        "display_name": "کلید دیفالت kwfinder",
        "description": "کلید پیش‌فرض Mangools (KWFinder) که برای تب «جستجوی کلمات کلیدی» استفاده می‌شود، مگر اینکه ابزار دیگری فعال کرده باشید.",
        "env_var": "MANGOOLS_API_KEY",
    },
}
...
def _sidebar_context(active_tab="api-keys"):
    ...
    return {
        ...
        "user": current_user,
        # این تب همیشه در دسترس است، حتی اگر دوره‌ی رایگان کاربر تمام شده باشد،
        # تا کاربر بتواند کلیدهای خودش را مدیریت کند.
        "trial_locked": False,
...
def index():
    ...
    default_keys = {
        "deepseek": {
            **DEFAULT_KEYS_INFO["deepseek"],
            "configured": bool(DEFAULT_DEEPSEEK_API_KEY),
            "masked_key": _mask_key(DEFAULT_DEEPSEEK_API_KEY),
        },
        "kwfinder": {
            **DEFAULT_KEYS_INFO["kwfinder"],
            "configured": bool(DEFAULT_MANGOOLS_API_KEY),
            "masked_key": _mask_key(DEFAULT_MANGOOLS_API_KEY),
        },
    }
