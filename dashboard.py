...
def trial_required(f):
    """جلوگیری از اجرای درخواست‌های ابزار سئو پیش از فعال‌سازی یا پس از پایان دوره‌ی رایگان."""
    @wraps(f)
...
def overview(tab="content-analyzer"):
    ...
    # اگر دوره‌ی رایگان کاربر فعال نشده یا تمام شده و اشتراکی هم ندارد، دیگر
    # درخواست‌های پرهزینه (جستجوی یوتیوب/گوگل/کلمات کلیدی) اجرا نمی‌شود؛ محتوای
    # پنل به‌صورت مات‌شده نمایش داده می‌شود و کاربر به صفحه‌ی پلن‌ها هدایت می‌شود.
    if trial_active:
